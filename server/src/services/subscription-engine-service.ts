...
class SubscriptionEngineService {
  createRenewalOrders(res: Response) {
    ...
    const nextRenewalDate = SubscriptionDateHelper.getNextCreateRenewalDate(
      nextDeliveryDate,
      Constants.subscriptionRenewalPaymentDaysBeforeDelivery
    );

    if (SubscriptionDateHelper.isTodayBeforeDate(nextRenewalDate)) {
      return res.send(
        `No renewal orders created, will not create any until ${nextRenewalDate.toString()}`
      );
    }

    // subscriptionRepo
    //   .getSubscriptions({ status: SubscriptionStatus.active })
    //   .then(subscriptions => {
    //     logger.info(
    //       `Start creating renewal orders for ${
    ...
    //   .catch(function(err) {
    //     logger.error(
    //       `Error when getting subscriptions, couldn't start creating renewal orders. ${
    //         err.message
    //       }`
    //     );
    //     return res.status(501).send(err.message);
  ...
  private createRenewalOrder_Step1 = function(
    subscription,
    nextDeliveryDate,
    nextRenewalDate
  ) {
    if (!this.isTimeForRenewalOrder(nextDeliveryDate, subscription)) {
      logger.debug(
        `Not yet time for renewal order for subscription id: ${subscription.id}`
      );
      return;
    }

    return orderService
      .getOrders({ subscriptionId: subscription.id })
      .then(orders => {
        if (orders.length > 0) {
          let match = orders.find(order =>
            SubscriptionDateHelper.isSameDate(order.orderDate, nextRenewalDate)
          );
          ...
          if (match.length === 1) {
            logger.debug(
              `Renewal order for ${nextRenewalDate.toString()} was already created. Subscription id: ${
                subscription.id
              }`
            );
          }
  ...
  private isTimeForRenewalOrder(nextDeliveryDate, subscription): Boolean {
    const isNextDeliveryFirstOfMonth = SubscriptionDateHelper.isDateFirstDeliveryOfMonth(
      nextDeliveryDate
    );
    return (
      subscription.frequence !== SubscriptionFrequence.monthly ||
      isNextDeliveryFirstOfMonth ||
      !SubscriptionDateHelper.isDateFirstDeliveryOfMonth(
