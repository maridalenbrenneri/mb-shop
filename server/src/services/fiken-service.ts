  ...
  private getHeaders() {
    return {
      Authorization:
        "Basic " +
        new Buffer(
          process.env.FIKEN_USER + ":" + process.env.FIKEN_PWD
        ).toString("base64"),
  ...
  async getCustomers() {
    ...
    return new Promise<any>(function (resolve, reject) {
      self.request(options, function (error: any, response: any) {
        ...
        const customers =
          res["_embedded"][`${process.env.FIKEN_BASE_URL}/rel/contacts`];

        const clientCustomers = customers
          .map((c) => self.mapToCustomerClientModel(c))
